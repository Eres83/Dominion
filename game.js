...
function loadState(){
  try{
    const s = localStorage.getItem("starDominionOnlineSave");
    if(s){
...
function saveState(){
  try{
    localStorage.setItem("starDominionOnlineSave", JSON.stringify(state));
...
function getBuildingData(){
  return {
    metalMine:{
      name:"Mine de métal",
      desc:"Augmente la production de métal.",
    ...
    crystalMine:{
      ...
      desc:"Augmente la production de cristal.",
    ...
    deuteriumSynth:{
      ...
      desc:"Produit du deutérium à partir de la mer.",
      baseCost:{metal:225, crystal:75},
      costFactor:1.5,
      depends:{solarPlant:1},
      category:"resources"
    },
    solarPlant:{
      name:"Centrale solaire",
      desc:"Fournit de l'énergie pour alimenter les mines.",
    ...
    roboFactory:{
      ...
      desc:"Réduit le temps de construction (symbolique ici).",
    ...
    researchLab:{
      ...
      desc:"Permet la recherche de nouvelles technologies.",
    ...
    shipyard:{
      ...
      desc:"Permet la construction de vaisseaux.",
      baseCost:{metal:400, crystal:200},
      costFactor:2.0,
      depends:{roboFactory:1},
      category:"facilities"
    },
    storageMetal:{
      name:"Hangar de métal",
      desc:"Augmente la capacité de stockage du métal.",
      baseCost:{metal:200, crystal:0},
      costFactor:1.8,
      depends:null,
      category:"facilities"
    },
    storageCrystal:{
      ...
      desc:"Augmente la capacité de stockage du cristal.",
      baseCost:{metal:150, crystal:50},
      costFactor:1.8,
      depends:null,
      category:"facilities"
    }
...
function getResearchData(){
  return {
    energyTech:{
      ...
      desc:"Améliore l'efficacité énergétique.",
      baseCost:{metal:0, crystal:400, deuterium:0},
      factor:2,
      depends:{researchLab:1}
    },
    combustionDrive:{
      name:"Propulsion à combustion",
      desc:"Permet de développer des vaisseaux plus rapides.",
    ...
    armorTech:{
      ...
      desc:"Renforce la solidité des vaisseaux.",
      baseCost:{metal:1000, crystal:0, deuterium:0},
      factor:2,
      depends:{researchLab:2}
    },
    weaponsTech:{
      name:"Technologie d'armement",
      desc:"Augmente la puissance de feu.",
...
function getShipData(){
  return {
    smallCargo:{
      ...
      desc:"Transport de ressources basique.",
      cost:{metal:2000, crystal:2000, deuterium:0}
    },
    lightFighter:{
      name:"Chasseur léger",
      desc:"Unité d'attaque rapide.",
      cost:{metal:3000, crystal:1000, deuterium:0}
    },
    recycler:{
      name:"Recycleur",
      desc:"Permet de récupérer des débris (symbolique).",
      cost:{metal:10000, crystal:6000, deuterium:2000}
    },
    probe:{
      name:"Sonde d'espionnage",
      desc:"Collecte des informations (placeholder).",
...
function renderBuildings(){
  ...
  Object.entries(data).forEach(([key,info])=>{
    ...
    const depsOk = info.depends ? Object.entries(info.depends).every(
      ([b,lvl])=>buildingLevel(b)>=lvl
    ):true;
...
function renderResearch(){
  ...
  Object.entries(data).forEach(([key,info])=>{
    ...
    if(!depsOk){
      const dep = document.createElement("div");
      dep.className="badge";
      dep.textContent="Prérequis: niveau labo insuffisant";
      card.appendChild(dep);
    }
...
function renderFleetTable(){
  ...
  Object.entries(state.ships).forEach(([key,qty])=>{
    ...
    nameTd.textContent = data[key]?.name || key;
...
function renderGalaxy(){
  ...
  for(let g=1; g<=4; g++){
    ...
    for(let p=1; p<=4; p++){
      ...
      planetDiv.appendChild(left);
      planetDiv.appendChild(right);
...
function generateBossWindow(){
  ...
  const attacks = [];
  const count = randInt(0,2); // rare : 0 à 2 boss / 48h
...
function getBossPower(attack){
  const base = getPlayerPower();
  const diff = attack.difficulty;
  let factor;
...
function processBossAttacks(){
  ...
  if(outcome==="win"){
    ...
    due.result = { outcome:"Victoire !", lootMetal:lootM, lootCrystal:lootC,
                   playerPower:Math.round(playerPower), bossPower:Math.round(bossPower)};
    ...
    due.result = { outcome:"Défaite…", lostMetal:lostM, lostCrystal:lostC, lostDeut:lostD,
                   playerPower:Math.round(playerPower), bossPower:Math.round(bossPower)};
...
function renderBossStatus(){
  ...
  const now = Date.now();
  const upcoming = state.enemy.bossAttacks
    .filter(a=>!a.resolved && a.timestamp>now)
  ...
  box.innerHTML = `
    <p><strong>Prochain boss détecté :</strong></p>
...
function renderBossLog(){
  ...
  state.enemy.bossLog.forEach(entry=>{
    ...
    t.className="time";
    t.textContent = new Date(entry.time).toLocaleTimeString("fr-FR");
    const ty = document.createElement("span");
    ty.className = "type "+(entry.type||"info");
    ty.textContent = entry.type||"info";
